...
Conv. function: $f: X\rightarrow \mathbb{R}$, $X$ convex: $\forall x,y\in X, \forall t \in [0,1] : f(tx+(1-t)y)\leq tf(x)+(1-t)f(y)$\\
Conv. set:$\forall \lambda >0, \sum_\lambda = 1, u_k\in S: \sum_{k=1}^{n}\lambda_k u_k \in S$\\
