...
\ge \sum_{n=1}^N \sum_{k=1}^K{q_{kn}[\log p_{\theta_k}(\mathbf{x}_n) + \log \pi_k - \log q_{kn}]}$\\
with $\forall n, \sum_{k=1}^K{q_{kn}} = 1$ by Jensen Inequality.
...
$\mathcal{N}(x|\mu, \sigma^2) = \frac{1}{\sqrt{2\pi \sigma^2}} e^{- \frac{1}{2} \frac{(x-\mu)^2}{\sigma^2}}$\\
Covariance matrix $\Sigma$, Mean $\mu$ , $D$-dimensional:\\
$\mathcal{N}_D(x| \mu, \Sigma) = \frac{1}{\sqrt{(2\pi)^D det(\Sigma)}} e^{- \frac{1}{2} (x-\mu)^T \Sigma^{-1} (x-\mu)}$
